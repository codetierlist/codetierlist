...
export const RawAchievementCard = ({ icon, description, name }: Omit<AchievementConfig, "id"|"config"|"type"|"depends_on">) => {
    return (
        <Card orientation="horizontal" className="p-y-none" style={{ height: 100 }}>
            ...
            <div>
                <Text as="h3" weight="semibold">{name}</Text>
                <br />
                <Caption1 as="p">
                    {description}
                </Caption1>
...
export const AchievementCard = ({ achievement }: { achievement: AchievementConfig }) => {
    return (
        <RawAchievementCard {...achievement} />
    );
};
